...
class App extends Component {
  ...
  fetchData = () => {
    Promise.all([this.fetchProducts(), this.fetchShoppingCart()])
      .then(([products, shoppingCart]) =>
  ...
  fetchProducts = () => {
    return fetch(`${config.API_ENDPOINT}/products`).then((res) => {
      if (!res.ok) {
  ...
  fetchShoppingCart = () => {
    return fetch(`${config.API_ENDPOINT}/shoppingCart`).then((res) => {
      if (!res.ok) {
        throw new Error(res.statusText);
      }
      return res.json();
